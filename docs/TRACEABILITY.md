...
| Post-publication integration-line return | IMPLEMENTED | every local pull-request creation returns the workspace to `develop` and reports `switched`, `already-home`, `skipped-dirty-worktree`, or `failed` without questioning the created pull request; the protected workflow-token and hotfix-propagation-publisher server contexts never switch their ephemeral checkout; workflow and bootstrap whitebox tests cover every transition state and the server-side exclusion |
