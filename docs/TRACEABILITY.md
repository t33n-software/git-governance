...
| Protected release-line dispatch | VERIFIED | Direct dispatch waits for a correlated successful child workflow, then fetches and verifies the remote line. The protected controller can defer only the child-approval wait with a non-secret correlation ID; a later explicit verification proves child success and the remote reference without duplicate dispatch. |
