...
| Dependency-review merge enforcement | VERIFIED | `Dependency review / Dependency admission review` is a required status check for `develop`, `main`, `release/*`, and `support/*` in the `linux-only` ruleset class variants (the `full` variants keep the inline-era `Dependency admission review` context until the full-class caller adoption); the repository contract test binds the workflow name and every shared-line ruleset class variant |
| Composite check-context migration | VERIFIED | the `linux-only` shared-line ruleset class variants bind the canonical composite contexts `Quality gates / linux-amd64` and `Dependency review / Dependency admission review` per the naming law; the contract test binds the composite contexts for the `linux-only` class and the inline-era contexts for the `full` class until its caller adoption |
