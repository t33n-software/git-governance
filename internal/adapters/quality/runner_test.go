...
func TestRunnerErrorAndScopeHelperPaths(t *testing.T) {
	...
	t.Run("unknown requested family", func(t *testing.T) {
		root := t.TempDir()
		writeConfig(t, filepath.Join(root, defaultConfigName), `{"schemaVersion":4,"toolchain":{"language":"go","version":"1.26.6"},"gates":[{"name":"test","command":"go"}]}`)
...
func TestRunRejectsUnsafeOrInvalidConfigurations(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name:     "unknown field",
			contents: `{"schemaVersion":4,"toolchain":{"language":"go","version":"1.26.6"},"gates":[{"name":"test","command":"go","unknown":true}]}`,
		},
		{
			name:     "empty gates",
			contents: `{"schemaVersion":4,"toolchain":{"language":"go","version":"1.26.6"},"gates":[]}`,
		},
		{
			name:     "escaping directory",
			contents: `{"schemaVersion":4,"toolchain":{"language":"go","version":"1.26.6"},"gates":[{"name":"test","command":"go","workingDirectory":"../outside"}]}`,
		},
		{
			name:     "shell control argument",
			contents: "{\"schemaVersion\":4,\"toolchain\":{\"language\":\"go\",\"version\":\"1.26.6\"},\"gates\":[{\"name\":\"test\",\"command\":\"go\",\"args\":[\"test\\n./...\"]}]}",
		},
		{
			name:     "unknown family",
			contents: `{"schemaVersion":4,"toolchain":{"language":"go","version":"1.26.6"},"gates":[{"name":"test","command":"go","includeFamilies":["unknown"]}]}`,
		},
		{
			name:     "scope overlap",
			contents: `{"schemaVersion":4,"toolchain":{"language":"go","version":"1.26.6"},"gates":[{"name":"test","command":"go","includeFamilies":["feature"],"excludeFamilies":["feature"]}]}`,
		},
		{
			name:     "old schema",
		...
		},
		{
			...
			contents: `{"schemaVersion":4,"toolchain":{"language":"go","version":"1.26.6"},"defaults":{"includeFamilies":["feature","feature"]},"gates":[{"name":"test","command":"go"}]}`,
		},
		{
			name:     "empty command",
			contents: `{"schemaVersion":4,"toolchain":{"language":"go","version":"1.26.6"},"gates":[{"name":"test","command":" "}]}`,
		},
		{
			name:     "multiple documents",
			contents: `{"schemaVersion":4,"toolchain":{"language":"go","version":"1.26.6"},"gates":[{"name":"test","command":"go"}]} {}`,
		},
		{
			name:     "project binary package not repository relative",
			contents: `{"schemaVersion":4,"toolchain":{"language":"go","version":"1.26.6"},"gates":[{"name":"test","command":"go"}],"project":{"binaries":[{"package":"cmd/tool"}]}}`,
		},
		{
			name:     "duplicate project binary package",
			contents: `{"schemaVersion":4,"toolchain":{"language":"go","version":"1.26.6"},"gates":[{"name":"test","command":"go"}],"project":{"binaries":[{"package":"./cmd/tool"},{"package":"./cmd/tool"}]}}`,
		},
		{
			name:     "smoke argument with control character",
			contents: "{\"schemaVersion\":4,\"toolchain\":{\"language\":\"go\",\"version\":\"1.26.6\"},\"gates\":[{\"name\":\"test\",\"command\":\"go\"}],\"project\":{\"binaries\":[{\"package\":\"./cmd/tool\",\"smoke\":[\"--version\\n\"]}]}}",
		},
		{
			name:     "project fuzz package not repository relative",
			contents: `{"schemaVersion":4,"toolchain":{"language":"go","version":"1.26.6"},"gates":[{"name":"test","command":"go"}],"project":{"fuzz":[{"package":"internal/boundary","target":"FuzzParse","time":"50000x"}]}}`,
		},
		{
			name:     "project fuzz target is not a fuzz name",
			contents: `{"schemaVersion":4,"toolchain":{"language":"go","version":"1.26.6"},"gates":[{"name":"test","command":"go"}],"project":{"fuzz":[{"package":"./internal/boundary","target":"Parse","time":"50000x"}]}}`,
		},
		{
			name:     "duplicate project fuzz target",
			contents: `{"schemaVersion":4,"toolchain":{"language":"go","version":"1.26.6"},"gates":[{"name":"test","command":"go"}],"project":{"fuzz":[{"package":"./internal/boundary","target":"FuzzParse","time":"50000x"},{"package":"./internal/boundary","target":"FuzzParse","time":"30s"}]}}`,
		},
		{
			name:     "project fuzz empty time",
			contents: `{"schemaVersion":4,"toolchain":{"language":"go","version":"1.26.6"},"gates":[{"name":"test","command":"go"}],"project":{"fuzz":[{"package":"./internal/boundary","target":"FuzzParse","time":""}]}}`,
		},
		{
			name:     "project fuzz invalid duration",
			contents: `{"schemaVersion":4,"toolchain":{"language":"go","version":"1.26.6"},"gates":[{"name":"test","command":"go"}],"project":{"fuzz":[{"package":"./internal/boundary","target":"FuzzParse","time":"abc"}]}}`,
		},
		{
			name:     "project fuzz empty execution count",
			contents: `{"schemaVersion":4,"toolchain":{"language":"go","version":"1.26.6"},"gates":[{"name":"test","command":"go"}],"project":{"fuzz":[{"package":"./internal/boundary","target":"FuzzParse","time":"x"}]}}`,
		},
		{
			name:     "project fuzz non-numeric execution count",
			contents: `{"schemaVersion":4,"toolchain":{"language":"go","version":"1.26.6"},"gates":[{"name":"test","command":"go"}],"project":{"fuzz":[{"package":"./internal/boundary","target":"FuzzParse","time":"5.5x"}]}}`,
		},
	}
...
func TestRunAcceptsTheCanonicalV4Configuration(t *testing.T) {
	t.Parallel()
  ...
  "gates": [
	...
	result, err := runner.Run(context.Background(), port.RepositoryIdentity{Root: root}, qualityRequest(branch.FamilyFeature))
	if err != nil {
		t.Fatalf("the canonical v4 configuration must run: %v", err)
	}
...
func TestDecodeAcceptsTheFullV4ProjectBlock(t *testing.T) {
	t.Parallel()
  ...
  "project": {
	...
	if err != nil {
		t.Fatalf("decode() rejected the full v4 form: %v", err)
	}
	if configuration.Toolchain.Language != "go" ||
		configuration.Toolchain.Version != "1.26.6" ||
		len(configuration.Extends) != 1 ||
		configuration.Extends[0] != "opentofu@1" ||
		...
		t.Fatalf("decode() = %#v", configuration)
	}
}
...
func TestDecodeRejectsPreviousSchemaAndInvalidToolchain(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			contents: `{"schemaVersion":3,"toolchain":{"language":"go","version":"1.26.6"},"gates":[{"name":"test","command":"go"}]}`,
			rule:     "schemaVersion must equal 4",
		},
		{
			name:     "missing toolchain fails closed",
			contents: `{"schemaVersion":4,"gates":[{"name":"test","command":"go"}]}`,
			rule:     "toolchain language must be a lowercase language identifier such as go",
		},
		{
			name:     "invalid toolchain language fails closed",
			contents: `{"schemaVersion":4,"toolchain":{"language":"Go","version":"1.26.6"},"gates":[{"name":"test","command":"go"}]}`,
			rule:     "toolchain language must be a lowercase language identifier such as go",
		},
		{
			name:     "missing toolchain version fails closed",
			contents: `{"schemaVersion":4,"toolchain":{"language":"go"},"gates":[{"name":"test","command":"go"}]}`,
			rule:     "toolchain version must be a pinned version such as 1.26.6",
		},
		{
			name:     "unpinned toolchain version fails closed",
			contents: `{"schemaVersion":4,"toolchain":{"language":"go","version":"latest"},"gates":[{"name":"test","command":"go"}]}`,
			rule:     "toolchain version must be a pinned version such as 1.26.6",
		},
		{
			name:     "prefixed toolchain version fails closed",
			contents: `{"schemaVersion":4,"toolchain":{"language":"go","version":"go1.26.6"},"gates":[{"name":"test","command":"go"}]}`,
			rule:     "toolchain version must be a pinned version such as 1.26.6",
		},
	}
	for _, testCase := range testCases {
...
func TestDecodeValidatesTheExtendsDeclaration(t *testing.T) {
	t.Parallel()

	t.Run("accepts absent empty and populated declarations", func(t *testing.T) {
		t.Parallel()
		for _, contents := range []string{
			`{"schemaVersion":4,"toolchain":{"language":"go","version":"1.26.6"},"gates":[{"name":"test","command":"go"}]}`,
			`{"schemaVersion":4,"toolchain":{"language":"go","version":"1.26.6"},"extends":[],"gates":[{"name":"test","command":"go"}]}`,
			`{"schemaVersion":4,"toolchain":{"language":"go","version":"1.26.6"},"extends":["opentofu@1","lint-go@2"],"gates":[{"name":"test","command":"go"}]}`,
		} {
			if _, err := decode("quality.json", []byte(contents)); err != nil {
				t.Fatalf("decode() rejected a valid extends declaration: %v", err)
			}
		}
	})
	...
	testCases := []struct {
		...
		{
			name:     "missing major version fails closed",
			contents: `{"schemaVersion":4,"toolchain":{"language":"go","version":"1.26.6"},"extends":["opentofu"],"gates":[{"name":"test","command":"go"}]}`,
			rule:     "extends entries must use the <capability>@<major> form such as opentofu@1",
		},
		{
			name:     "unpinned major version fails closed",
			contents: `{"schemaVersion":4,"toolchain":{"language":"go","version":"1.26.6"},"extends":["opentofu@latest"],"gates":[{"name":"test","command":"go"}]}`,
			rule:     "extends entries must use the <capability>@<major> form such as opentofu@1",
		},
		{
			name:     "uppercase capability fails closed",
			contents: `{"schemaVersion":4,"toolchain":{"language":"go","version":"1.26.6"},"extends":["OpenTofu@1"],"gates":[{"name":"test","command":"go"}]}`,
			rule:     "extends entries must use the <capability>@<major> form such as opentofu@1",
		},
		{
			name:     "empty capability fails closed",
			contents: `{"schemaVersion":4,"toolchain":{"language":"go","version":"1.26.6"},"extends":["@1"],"gates":[{"name":"test","command":"go"}]}`,
			rule:     "extends entries must use the <capability>@<major> form such as opentofu@1",
		},
		{
			name:     "duplicate reference fails closed",
			contents: `{"schemaVersion":4,"toolchain":{"language":"go","version":"1.26.6"},"extends":["opentofu@1","opentofu@1"],"gates":[{"name":"test","command":"go"}]}`,
			rule:     "extends entries must be unique",
		},
	}
	for _, testCase := range testCases {
		testCase := testCase
...
func FuzzDecodeQualityConfiguration(f *testing.F) {
	for _, seed := range []string{
		`{"schemaVersion":4,"toolchain":{"language":"go","version":"1.26.6"},"gates":[{"name":"unit-tests","command":"go","args":["test","./..."],"timeout":"2m"}]}`,
		`{"schemaVersion":4,"toolchain":{"language":"go","version":"1.26.6"},"defaults":{"includeFamilies":["feature"]},"gates":[{"name":"stress","command":"tool","includeFamilies":["perf"]}]}`,
		`{"schemaVersion":4,"toolchain":{"language":"go","version":"1.26.6"},"extends":["opentofu@1"],"gates":[{"name":"full","command":"go"}],"project":{"binaries":[{"package":"./cmd/tool","smoke":["--version"]}],"fuzz":[{"package":"./internal/boundary","target":"FuzzParse","time":"50000x"}]}}`,
		`{"schemaVersion":3,"toolchain":{"goVersion":"1.26.6"},"gates":[{"name":"unit-tests","command":"go"}]}`,
		`{"schemaVersion":4,"toolchain":{"language":"go","version":"1.26.6"},"gates":[]}`,
		`{"schemaVersion":4,"toolchain":{"language":"go","version":"1.26.6"},"gates":[{"name":"test","command":"go","workingDirectory":"../outside"}]}`,
		`{`,
		"",
		"\x00",
	} {
